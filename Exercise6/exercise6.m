...
fun = @(x) N * (x(1) * log(x(2)) - log(x(1))) - x(1) ...
    * sum(log(v_w)) + (x(2) ^ -x(1)) * sum(v_w .^ x(1));

% options for fmincon
options = optimoptions('fmincon','Display','iter');
% call fmincon to solve the problem
[theta,y] = fmincon(fun,theta0,[],[],[],[],[0 0],[inf inf],[],options);
...
Weibull = @(v) pDist(v, kStar, lambdaStar);
...
for i=1:26
   expectedPower = expectedPower + power(i) * windSpeedProbability(i);
end
% expectedPower = expectedPower - 0.5 * power(26) * Weibull(26);
